...
namespace BeautySCProject.API.Controllers
{
    public class RoutineController : BaseAPIController
    {
        ...
        [HttpGet("get-routine-by-routine-id-admin")]
        public async Task<IActionResult> GetRoutineByRoutineIdForAdmin(int routineId)
        {
            var result = await _routineService.GetRoutineByRoutineIdForAdminAsync(routineId);
            return result.Match(
                (l, c) => Problem(detail: l, statusCode: c),
                Ok
            );
